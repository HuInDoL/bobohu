...
- [아키텍처](architecture.md): 시스템 구성도, 저장소 구조, 서버 모듈 구조, 주요 흐름(후원 결제 등), 보안 아키텍처 (뼈대)
- [데이터 모델](data-model.md): 테이블 설계와 컬럼 선택 이유 (facility)
...
- [Spring Security](tech/spring-security.md): 인증과 인가, 필터 체인, 기본값 거부 설정, CSRF를 끈 이유
- [Flyway](tech/flyway.md): DB 마이그레이션. `ddl-auto: validate`와 역할 분담
- [Docker Compose](tech/docker-compose.md): 로컬 개발용 PostgreSQL, `.env`로 비밀 정보 분리
- [Testcontainers](tech/testcontainers.md): 실제 PostgreSQL로 통합 테스트
...
- [후원](features/donation.md): 법적 검토, 직접 연결 방식, 후원 및 사용 내역 원장 기록 (예정)
